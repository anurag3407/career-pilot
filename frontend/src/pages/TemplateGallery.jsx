...
/* ─── Sandboxed preview frame (fixes fixed-nav bleed-out) ──────────────────── */
  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        ...
        className={`flex items-center justify-between gap-3 min-w-[160px] px-4 py-2.5 rounded-xl border text-sm font-medium text-foreground bg-card backdrop-blur-sm transition-all duration-300 cursor-pointer select-none ${
          open
            ...
            {options.map((opt) => {
              ...
              return (
                <li
                  ...
                  className={`flex items-center justify-between gap-3 px-4 py-2.5 text-sm cursor-pointer select-none transition-all duration-200 ${
                    isSelected
...
export default function TemplateGallery() {
  ...
  const filteredTemplates = templates.filter((t) => {
    ...
    return (
      ...
      (colorScheme === "All" || t.colorScheme === colorScheme) &&
      (layout === "All" || t.layout === layout)
    );
  });
