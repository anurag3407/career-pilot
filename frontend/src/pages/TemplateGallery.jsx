  ...
  return (
    <div className="mt-12">
      ...
      <div
        ...
        style={{ height: 600, overflowY: 'auto', overflowX: 'hidden', transform: 'translate(0)', position: 'relative' }}
      >
        {children}
      </div>
    </div>
...
function TemplateCard({ template, hovered, onHover, onLeave, onUse, aiDraft }) {
  ...
  return (
    <motion.div
      ...
      initial="rest"
      variants={{
        rest: { y: 0, scale: 1, boxShadow: '0 4px 20px rgba(0,0,0,0.25)', borderColor: 'rgba(255,255,255,0.08)', transition: { duration: 0.4, ease: [0.25, 0.46, 0.45, 0.94] } },
        hover: { y: -10, scale: 1.02, boxShadow: '0 28px 52px rgba(0,0,0,0.50), 0 0 0 1px rgba(99,102,241,0.55)', borderColor: 'rgba(99,102,241,0.65)', transition: { type: 'spring', stiffness: 280, damping: 22 } },
      }}
      ...
      <div className="overflow-hidden relative bg-background aspect-[16/10]">
        ...
        {shouldRenderIframe && (
          <div
            ...
            style={{ width: '500%', height: '500%', transform: 'scale(0.2)', opacity: iframeLoaded ? 1 : 0, transition: 'opacity 0.6s ease-in-out' }}
          >
            <iframe
...
export default function TemplateGallery() {
  ...
  useEffect(() => {
    ...
    if (draft) {
      try { setAiDraft(JSON.parse(draft)); } catch(e) {}
    }
  }, []);
      ...
      <div className="p-8 pt-24">
        {aiDraft && (
          <div className="mb-8 p-4 rounded-xl bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border border-emerald-500/30 flex items-center justify-between">
            ...
            <button onClick={clearDraft} className="p-2 hover:bg-emerald-500/20 text-emerald-400 rounded-lg transition-colors" title="Discard Draft">
              <X className="w-5 h-5" />
            </button>
          </div>
        )}
