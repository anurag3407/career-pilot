      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <motion.div
            ...
            className="relative -mt-20 rounded-3xl bg-card/60 backdrop-blur-md border border-border p-6 sm:p-8"
          >
            <div className="flex flex-col md:flex-row items-center md:items-start gap-6 text-center md:text-left">
              {/* Initials Avatar */}
              <div
                className={`w-28 h-28 rounded-3xl bg-gradient-to-br ${avatarGradient} flex items-center justify-center flex-shrink-0 shadow-xl border-4 border-card`}
              >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div>
                    <h1 className="text-3xl font-extrabold text-foreground tracking-tight">{displayName}</h1>
                    {profile?.jobRole && (
                    ...
                    <div className="flex items-center gap-2">
                      {editing ? (
                        ...
                        <Button
                          variant="secondary"
                          onClick={startEdit}
                          className="!py-2.5 !px-5 text-xs font-bold border-border"
                        >
                ...
                {!editing && (profile?.website || profile?.github || profile?.linkedin) && (
                  <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mt-4 pt-4 border-t border-border">
                    {profile.website && (
                      <a
          ...
          <motion.div variants={itemVariants} className="flex flex-wrap gap-2 p-1.5 rounded-2xl bg-card/40 border border-border backdrop-blur-sm overflow-x-auto">
            {visibleTabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 ${
                    isActive
                      ? 'text-foreground bg-indigo-500/10 border border-indigo-500/20'
                      : 'text-zinc-400 hover:text-foreground hover:bg-muted/30 border border-transparent'
                  }`}
                >
          ...
          <div className="min-h-[400px]">
            <AnimatePresence mode="wait">
              {activeTab === 'workspace' && (
                ...
                >
                  {/* Bio and About info */}
                  <div className="rounded-3xl bg-card/40 border border-border p-6 sm:p-8 space-y-6">
                    <div>
                      ...
                      {editing ? (
                        <div>
                          <textarea
                            ...
                            className="w-full px-5 py-3.5 rounded-2xl bg-neutral-950/60 border border-border text-foreground placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500/70 resize-none transition-all duration-200"
                          />
                        ...
                        <p className="text-zinc-350 text-sm sm:text-base leading-relaxed">{profile.bio}</p>
                      ) : isOwnProfile ? (
                    ...
                    {editing && (
                      <div className="grid sm:grid-cols-2 gap-x-6 gap-y-1 mt-4 pt-4 border-t border-border">
                        <Input
                  ...
                  {!editing && profile?.skills?.length > 0 && (
                    <div className="rounded-3xl bg-card/40 border border-border p-6 sm:p-8">
                ...
                >
                  ...
                  {recsLoading ? (
                    <div className="grid md:grid-cols-2 gap-4">
                      {[1, 2, 3, 4].map((i) => (
                        <div key={i} className="animate-pulse rounded-3xl bg-card/40 border border-border p-6 h-48" />
                      ))}
                    </div>
                  ) : recommendations.length === 0 ? (
                    <div className="rounded-3xl bg-card/30 border border-border p-8 sm:p-12 text-center space-y-4">
                      ...
                      <p className="text-zinc-450 text-sm max-w-md mx-auto">
                        Add target Job Roles and Skills on your profile Workspace to unlock custom AI recommendation listings.
                      </p>
                      <Button onClick={() => setActiveTab('workspace')} variant="outline" className="text-xs !py-2.5 border-border">
                        Update Workspace
                      </Button>
                    </div>
                  ) : (
                    <div className="grid md:grid-cols-2 gap-4">
                      {recommendations.map((job, idx) => {
                        ...
                        return (
                          <div
                            key={idx}
                            className="group flex flex-col justify-between rounded-3xl bg-card/40 border border-border p-6 hover:border-indigo-500/30 transition-all-300"
                          >
                            <div className="space-y-4">
                              <div className="flex items-start gap-4">
                                {job.employer_logo ? (
                                  ...
                                  <div className="w-12 h-12 rounded-xl bg-muted flex items-center justify-center text-muted-foreground font-bold border border-border">
                                    {(job.employer_name || 'J').charAt(0).toUpperCase()}
                                  </div>
                                )}
                                <div className="min-w-0">
                                  <h4 className="text-base font-bold text-foreground truncate group-hover:text-indigo-400 transition-colors">
                                    {job.job_title}
                                  </h4>
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                ...
                                {job.job_city && (
                                  <span className="px-2 py-0.5 rounded-lg bg-muted border border-border text-zinc-400 text-[10px] font-bold">
                                    {job.job_city}, {job.job_state || job.job_country}
                                  </span>
                                )}
                              </div>

                              <p className="text-zinc-450 text-xs line-clamp-2 leading-relaxed font-medium">
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h2 className="text-xl font-bold text-foreground">Resume Hub</h2>
                  ...
                  {resumesLoading ? (
                    <div className="space-y-3">
                      {[1, 2].map((i) => (
                        <div key={i} className="animate-pulse rounded-2xl bg-card/40 border border-border p-5 h-20" />
                      ))}
                    </div>
                  ) : resumes.length === 0 ? (
                    <div className="rounded-3xl bg-card/30 border border-border p-8 sm:p-12 text-center space-y-4">
                      ...
                      <p className="text-zinc-450 text-sm max-w-md mx-auto font-medium">
                        Upload your PDF resume to generate career scores and optimization insights.
                      </p>
                      <Link to="/upload">
                        <Button variant="outline" className="text-xs !py-2.5 border-border">
                          Upload Now
                        </Button>
                      </Link>
                    </div>
                    ...
                    <div className="space-y-3">
                      {resumes.map((resume) => (
                        <div
                          key={resume.id}
                          className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-2xl bg-card/30 border border-border hover:border-zinc-500/25 transition-colors"
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h4 className="font-bold text-foreground text-sm">{resume.title}</h4>
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <Link to={`/enhance/${resume.id}`}>
                              <Button variant="outline" className="text-[11px] !py-2 !px-3 font-bold border-border">
                  ...
                  {/* Dynamic CTA Box */}
                  <div className="rounded-3xl bg-gradient-to-br from-indigo-950/20 via-zinc-900/40 to-purple-950/20 border border-border p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="space-y-2 text-center md:text-left">
                      <h3 className="font-bold text-foreground text-base flex items-center justify-center md:justify-start gap-2">
                        <Sparkles className="w-4 h-4 text-indigo-400" />
                        AI-Powered Resume Optimization
                      </h3>
                      <p className="text-zinc-450 text-xs max-w-lg leading-relaxed font-medium">
              ...
              {activeTab === 'tracker' && (
                ...
                >
                  <div className="grid grid-cols-2 gap-4">
                    <div className="rounded-3xl bg-card/40 border border-border p-5 text-center shadow-sm">
                      ...
                      <p className="text-3xl font-extrabold text-foreground">{stats.resumesCreated}</p>
                    ...
                    <div className="rounded-3xl bg-card/40 border border-border p-5 text-center shadow-sm">
                      ...
                      <p className="text-3xl font-extrabold text-foreground">{stats.interviewsDone}</p>
                  ...
                  {/* Career Action CTA */}
                  {isOwnProfile && (
                    <div className="rounded-3xl bg-card/40 border border-border p-6 sm:p-8 space-y-4">
                      <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-widest opacity-80">Quick Navigators</h3>
                      <div className="grid sm:grid-cols-2 gap-3">
                        <Link to="/interview-prep">
                          <div className="flex items-center justify-between p-4 rounded-2xl bg-muted/40 border border-border hover:border-indigo-500/30 transition-colors group cursor-pointer">
                            <div className="flex items-center gap-3">
                              <div className="p-2 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-400">
                                <Mic className="w-4 h-4" />
                              </div>
                              <span className="font-bold text-xs text-foreground">Interactive AI Interview Prep</span>
                            </div>
                            <ArrowRight className="w-4 h-4 text-zinc-655 group-hover:text-foreground transition-colors" />
                          </div>
                        </Link>
                        ...
                        <Link to="/job-tracker">
                          <div className="flex items-center justify-between p-4 rounded-2xl bg-muted/40 border border-border hover:border-indigo-500/30 transition-colors group cursor-pointer">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="font-bold text-xs text-foreground">Visual Job Search Tracker</span>
                            </div>
                            <ArrowRight className="w-4 h-4 text-zinc-655 group-hover:text-foreground transition-colors" />
                          </div>
                        </Link>
                  ...
                  <div className="space-y-4">
                    ...
                    {activity.length === 0 ? (
                      <div className="rounded-3xl bg-card/30 border border-border p-8 text-center space-y-3">
                        <MessageSquare className="w-10 h-10 text-zinc-700 mx-auto" />
                        <h4 className="font-bold text-foreground text-sm">No Posts Published</h4>
                        <p className="text-zinc-500 text-xs max-w-sm mx-auto">
                          Share thoughts or search queries in the community channels to log activities here.
                        </p>
                      </div>
                    ) : (
                      <div className="rounded-3xl bg-card/30 border border-border overflow-hidden divide-y divide-border/40">
                        {activity.map((item) => (
                          <div key={item.id} className="p-5 hover:bg-muted/20 transition-colors">
                            {item.title && <h4 className="font-bold text-foreground text-sm">{item.title}</h4>}
                            {item.content && <p className="text-zinc-450 text-xs mt-1 line-clamp-2 leading-relaxed font-medium">{item.content}</p>}
                            <div className="flex items-center gap-4 mt-3">
                              {item.category && (
                                <span className="px-2 py-0.5 rounded bg-muted border border-border text-zinc-400 text-[10px] font-bold">
                                  {item.category}
                                </span>
                              )}
              ...
              {activeTab === 'preferences' && (
                ...
                >
                  ...
                  {prefLoading ? (
                    <div className="animate-pulse rounded-3xl bg-card/40 border border-border p-6 h-56" />
                  ) : (
                    <div className="rounded-3xl bg-card/40 border border-border p-6 sm:p-8 space-y-6">
                      ...
                      <div className="flex items-center justify-between py-4 border-b border-border">
                        <div className="space-y-1">
                          <p className="text-foreground text-sm font-bold">Job Openings</p>
                      ...
                      <div className="flex items-center justify-between py-4 border-b border-border">
                        <div className="space-y-1">
                          <p className="text-foreground text-sm font-bold">Direct Messaging</p>
                      ...
                      <div className="flex items-center justify-between py-4">
                        <div className="space-y-1">
                          <p className="text-foreground text-sm font-bold">Fellowship & Proposals</p>
