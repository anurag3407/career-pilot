...
export default function ResumeBuilder() {
 ...
 // ─────────────────── ATS Keyword Assessment Loop ───────────────────
  useEffect(() => {
    ...
    const foundKeywords = ATS_KEYWORDS.filter(keyword =>
      resumeText.includes(keyword.toLowerCase())
    );

    const missing = ATS_KEYWORDS.filter(
    ...
    if (ATS_KEYWORDS.length > 0) {
      setAtsScore(
        Math.round((foundKeywords.length / ATS_KEYWORDS.length) * 100)
      );
    }
