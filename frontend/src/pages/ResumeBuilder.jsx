...
/**
 * A subtle clipboard button that sits in the top-right corner of a `relative
 * group` wrapper.  It is invisible until the user hovers the parent element,
 * keeping the UI clean.  On click it writes `getText()` to the clipboard and
 * shows a brief "Copied!" confirmation using the existing toast component.
 *
 * Usage:
 *   <div className="relative group">
 *     <textarea … />
 *     <CopyButton getText={() => someValue} />
 *   </div>
 */
function CopyButton({ getText, label = 'Copy' }) {
  ...
  return (
    <button
      ...
      aria-label={label}
      className={cn(
        // Position: sits in the top-right corner of the parent wrapper
        'absolute top-2 right-2 z-10',
        // Layout
        'flex items-center gap-1 px-2 py-1 rounded-md text-xs',
        // Appearance
        'bg-background/90 border border-border text-muted-foreground',
        // Hover-only visibility — show on parent group hover OR own focus
        'opacity-0 group-hover:opacity-100 focus:opacity-100',
        // Interaction
        'transition-opacity duration-150 hover:text-primary hover:border-primary',
      ...
      case 0:
        return (
          <div className="space-y-4">
            ...
            {/* ── Summary with CopyButton ── */}
            <div className="pt-2">
              <label className="block text-sm font-medium mb-1" htmlFor="summary">
                Professional Summary
              </label>
              {/* `relative group` lets CopyButton detect hover on the wrapper */}
              <div className="relative group">
                <textarea
                  id="summary"
              ...
              return (
                <div key={index} className="bg-background/30 p-5 rounded-xl border border-border relative">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {/* ── Education Description with CopyButton ── */}
                    <div className="md:col-span-2">
                      ...
                      <div className="relative group">
                        <textarea
                          className="w-full bg-muted border border-border rounded-lg px-4 py-2 min-h-[80px] focus:outline-none focus:ring-2 focus:ring-primary/30 transition-colors"
              ...
              return (
                <div key={index} className="bg-background/30 p-5 rounded-xl border border-border relative">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {/* ── Experience Description with CopyButton ── */}
                    <div className="md:col-span-2">
                      <label className="block text-sm font-medium mb-1">
                      ...
                      <AchievementEnhancer
                        value={exp.description}
                        jobRole={targetRole}
                        onApply={(text) => updateExp(index, "description", text)}
                      />
                    </div>
            ...
            {projects.map((proj, index) => (
              <div key={index} className="bg-background/30 p-5 rounded-xl border border-border relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {/* ── Project Description with CopyButton ── */}
                  <div className="md:col-span-2">
        ...
        return (
          <div className="space-y-4">
            ...
            <div>
              ...
              {/* ── Skills textarea with CopyButton ── */}
              <div className="relative group">
                <textarea
                  id="skills"
      ...
      case 5:
        return (
          <div className="space-y-6">
            ...
            <div className="mb-6 p-4 rounded-xl border border-border bg-muted">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold">Achievement Impact Score</h3>
                <span className="text-primary font-bold">{achievementScore}/100</span>
              </div>
              <div className="w-full bg-secondary rounded-full h-3">
                <div
            ...
            <div className="mb-6 p-4 rounded-xl border border-border bg-muted">
              <h3 className="font-semibold mb-3">Section Completion Status</h3>
            ...
            <div className="mb-6 p-4 rounded-xl border border-border bg-muted">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold">Resume Improvement Progress</h3>
                <span className="text-primary font-bold">{resumeScore}%</span>
              </div>
              <div className="w-full bg-secondary rounded-full h-3">
                <div
                  className="bg-primary h-3 rounded-full transition-all duration-500"
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <span className={`px-3 py-1 rounded-full text-sm ${resumeScore >= 20 ? "bg-green-500/20 text-green-500" : "bg-secondary"}`}>Personal Info</span>
                <span className={`px-3 py-1 rounded-full text-sm ${resumeScore >= 40 ? "bg-green-500/20 text-green-500" : "bg-secondary"}`}>Education</span>
                <span className={`px-3 py-1 rounded-full text-sm ${resumeScore >= 60 ? "bg-green-500/20 text-green-500" : "bg-secondary"}`}>Experience</span>
                <span className={`px-3 py-1 rounded-full text-sm ${resumeScore >= 80 ? "bg-green-500/20 text-green-500" : "bg-secondary"}`}>Projects</span>
                <span className={`px-3 py-1 rounded-full text-sm ${resumeScore >= 100 ? "bg-green-500/20 text-green-500" : "bg-secondary"}`}>Skills</span>
              </div>
              <p className="mt-2 text-sm text-muted-foreground">Complete more sections to improve your resume score.</p>
            </div>
              ...
              <div className="mb-6 p-4 rounded-xl border border-border bg-muted">
                ...
                <div className="space-y-2">
                  {resumeVersions.map(version => (
                    <div key={version.id} className="flex justify-between items-center p-2 rounded-lg bg-background">
                      <span className="text-sm">{version.timestamp}</span>
                      <button onClick={() => restoreVersion(version)} className="text-primary text-sm font-medium">
                        Restore
                      </button>
                    </div>
                  ))}
            ...
            <div className="mb-6 p-4 rounded-xl border border-border bg-muted">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold">Portfolio Social Profile Score</h3>
                <span className="text-primary font-bold">{profileScore}/100</span>
              </div>
              <div className="w-full bg-secondary rounded-full h-3">
                <div className="bg-primary h-3 rounded-full" style={{ width: `${profileScore}%` }} />
              </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {personal.linkedin && <span className="px-2 py-1 bg-green-500/20 text-green-500 rounded">LinkedIn Added</span>}
              {personal.github && <span className="px-2 py-1 bg-green-500/20 text-green-500 rounded">GitHub Added</span>}
              {personal.portfolio && <span className="px-2 py-1 bg-green-500/20 text-green-500 rounded">Portfolio Added</span>}
            </div>

            <div className="mb-6 p-4 rounded-xl border border-border bg-muted">
              <h3 className="font-semibold mb-4">Resume Content Impact Score</h3>
              {Object.entries(impactScores).map(([section, score]) => (
            ...
            <div className="mb-6 p-4 rounded-xl border border-border bg-muted">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold">Resume Readability Score</h3>
                <span className="text-primary font-bold">{readabilityScore}/100</span>
              </div>
              <div className="w-full bg-secondary rounded-full h-3">
                <div className="bg-primary h-3 rounded-full transition-all" style={{ width: `${readabilityScore}%` }} />
              </div>
            ...
            <div className="mb-6 p-4 rounded-xl border border-border bg-muted">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold">Resume Language Tone Analyzer</h3>
                <span className="text-primary font-bold">{toneScore}/100</span>
              </div>
              <div className="w-full bg-secondary rounded-full h-3">
                <div className="bg-primary h-3 rounded-full" style={{ width: `${toneScore}%` }} />
              </div>
              {toneSuggestions.length > 0 && (
                <div className="mt-4">
                  <h4 className="font-medium mb-2">Professional Tone Suggestions</h4>
                  <ul className="list-disc list-inside text-sm text-muted-foreground">
            ...
            {activeConsistencyWarnings.some(item => item.type === "duplicate") && (
              <div className="mb-6 p-4 rounded-xl border border-yellow-500 bg-yellow-500/10">
                <h3 className="font-semibold mb-3">Duplicate Information Detector</h3>
                <ul className="space-y-2">
                  {activeConsistencyWarnings
                    .filter(item => item.type === "duplicate")
                    .map((item, index) => (
                      <li key={index} className="text-sm text-yellow-400">• {item.message}</li>
                    ))}
                </ul>
              </div>
            )}
            ...
            <div className="mb-6 p-4 rounded-xl border border-border bg-muted">
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold">Career Goal Progress Tracker</h3>
                <span className="text-primary font-bold">{goalProgress}%</span>
              </div>
              <div className="w-full bg-secondary rounded-full h-3">
                <div className="bg-primary h-3 rounded-full" style={{ width: `${goalProgress}%` }} />
              </div>
              <div className="mt-4 space-y-2">
                {careerGoals.map((goal, index) => (
                  <div key={index}>{goal.completed ? "✅" : "⭕"} {goal.title}</div>
                ))}
              </div>
            </div>

            {/* ── Preview box with CopyButton for full resume text ── */}
            <div className="relative group">
