...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Upcoming interviews — feature/interview-scheduler
      const interviewJobs = jobs
        .filter(j => j.status === 'interviewing' && j.interviewDate)
        .sort((a, b) => new Date(a.interviewDate) - new Date(b.interviewDate))
        .slice(0, 3)
      setUpcomingInterviews(interviewJobs)

      // Portfolio count — main
      const portfolios =
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading ? (
          ...
          >
            ...
            >
              {[
                { to: '/jobs', icon: Search, label: 'Find Jobs', sub: 'Opportunities' },
                { to: '/job-alerts', icon: Bell, label: 'Alerts', sub: 'Get notified' },
                { to: '/interview-prep', icon: Mic, label: 'Interview', sub: 'AI practice' },
                { to: '/upload', icon: Sparkles, label: 'AI Enhance', sub: 'Optimize' },
                { to: '/job-tracker', icon: Briefcase, label: 'Tracker', sub: `${jobStats.total} tracked` },
                { to: '/community', icon: Users, label: 'Community', sub: 'Connect' },
                { to: '/fellowship', icon: GraduationCap, label: 'Fellowship', sub: 'Earn & learn' }
              ].map((action, idx) => (
                <motion.div key={idx} variants={itemVariants}>
                  <Link to={action.to} className="group">
                    <div className="relative p-5 rounded-2xl bg-card border border-border overflow-hidden transition-all duration-300 hover:border-primary hover:shadow-xl hover:-translate-y-1">
                      {action.isNew && (
                        <div className="absolute top-2 right-2 px-1.5 py-0.5 bg-primary/20 rounded text-[9px] text-primary font-black uppercase tracking-wider">
                          NEW
                        </div>
                      )}
            ...
            <motion.div
              variants={itemVariants}
              className="grid grid-cols-2 md:grid-cols-6 gap-5 mb-10"
            >
              {[
                ...
                { icon: Globe,       value: portfolioCount,       label: 'Portfolios',   color: 'text-purple-500',       bg: 'bg-purple-500/10', link: '/portfolio' }
              ].map((stat, idx) => {
                const content = (
              ...
              {trackedJobs.length === 0 ? (
                <div className="rounded-[2rem] bg-card/50 border border-border text-center py-16 backdrop-blur-sm">
                  ...
                  <p className="text-muted-foreground text-sm mb-6 max-w-xs mx-auto font-medium">
                    Start searching for jobs to track your applications
                  </p>
                  <Link to="/jobs">
                  ...
                  <div className="divide-y divide-border">
                    {trackedJobs.slice(0, 5).map((job, index) => {
                      ...
                      return (
                        <div
                          key={job.id || index}
                          className="p-5 hover:bg-muted/50 transition-all group"
                        >
