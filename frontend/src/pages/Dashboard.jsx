...
import { SkeletonDashboardActions, SkeletonStatCards, SkeletonList } from '../components/Skeletons'
  ...
  useEffect(() => {
    const checkUserStatus = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE}/users/onboarding/${user.uid}/status`, {
        ...
        if (response.ok) {
          ...
          const completedFlag = localStorage.getItem(`onboarding_completed_${user.uid}`);
              ...
              {[
                ...
                <motion.div key={idx} variants={itemVariants}>
                  <Link to={action.to} className="group">
                    <div className="relative p-5 rounded-2xl bg-card border border-border overflow-hidden transition-all duration-300 hover:-translate-y-1">
                      {action.isNew && (
