...
export default function PostEditor({ onClose, onSubmit, editPost = null }) {
  ...
  const [scheduledAt, setScheduledAt] = useState(editPost?.scheduledAt || null);
  const [showPoll, setShowPoll] = useState(Boolean(editPost?.poll));
  ...
  const handleSubmit = async (e) => {
    ...
    if (showPoll && pollOptions.filter(Boolean).length < 2) {
  ...
  return (
    <div className="fixed inset-0 bg-background/80 flex items-center justify-center z-50 p-4">
          ...
          <div className="p-6 space-y-5">
            ...
            <div>
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                <p className="text-xs text-muted-foreground">{content.length}/10000</p>
              </div>
            </div>
            ...
            <div>
              ...
              />
              <p className="mt-1 text-xs text-muted-foreground">Tags help others find your post</p>
            </div>
      ...
      {showScheduler && (
        <SchedulePost onClose={() => setShowScheduler(false)} onSchedule={handleScheduleConfirm} />
      )}
    </div>
  );
}
