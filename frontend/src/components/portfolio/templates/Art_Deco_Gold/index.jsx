...
const normalizeSkills = (skills, fallbackSkills) => {
  ...
  return skills.filter((skill) => skill && skill.name).map((skill) => ({
    name: skill.name,
...
export default function ArtDecoGold({ portfolioData }) {
  ...
  const socials = { ...fallbackData.socials, ...(source.socials || {}) };
  const stats = { ...fallbackData.stats, ...(source.stats || {}) };
  ...
  const experience = Array.isArray(source.experience) && source.experience.length ? source.experience : fallbackData.experience;
  const testimonials = Array.isArray(source.testimonials) && source.testimonials.length ? source.testimonials : fallbackData.testimonials;
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-slate-950 text-amber-100 font-sans" style={{ fontFamily: 'Georgia, Cambria, "Times New Roman", serif' }}>
