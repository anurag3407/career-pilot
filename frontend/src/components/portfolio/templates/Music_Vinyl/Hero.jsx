...
export default function Hero() {
  ...
  useEffect(() => {
    if (!playing) { cancelAnimationFrame(rafRef.current); prevTime.current = null; return; }
    const step = (ts) => {
      if (prevTime.current !== null) setAngle(a => (a + (ts - prevTime.current) * 0.04) % 360);
      prevTime.current = ts;
  ...
  return (
    <section className="relative w-full min-h-screen bg-zinc-950 overflow-hidden">
      ...
      <div className="relative z-10 flex items-center gap-3 px-8 md:px-16 pt-8">
        ...
        <span className="text-amber-700 text-xs font-mono tracking-widest uppercase">Side A · Portfolio Record 2025</span>
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-12 lg:gap-8 px-8 md:px-16 py-12 items-center min-h-[90vh]">
        ...
        <div className={`flex flex-col gap-8 transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <div className="flex items-center gap-2 w-fit bg-red-950 border border-red-800 rounded-full px-4 py-2">
            ...
            <span className="text-red-300 text-xs font-mono tracking-widest uppercase">Now Playing</span>
          </div>

          {/* NAME */}
          <div>
            <p className="text-amber-500 text-xs font-mono tracking-[0.3em] uppercase mb-3">— Track 01, Side A</p>
            ...
            <div className="flex items-center gap-3 mt-4">
              ...
              <p className="text-zinc-400 text-xs font-mono tracking-widest uppercase">Full Stack Developer</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3">
            <button className="flex items-center gap-2 bg-red-700 hover:bg-red-600 text-white text-sm font-semibold px-6 py-3 rounded-full transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-red-900/50">
              <Download size={15} />
              Download Resume
            </button>
            <button className="flex items-center gap-2 border border-zinc-700 hover:border-amber-600 text-zinc-300 hover:text-amber-400 text-sm font-semibold px-6 py-3 rounded-full transition-all duration-300">
          ...
          {/* STATS */}
          <div className={`grid grid-cols-3 gap-4 pt-6 border-t border-zinc-800 transition-all duration-700 delay-200 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            {[
              { v: "3+",  l: "Years Exp."  },
              { v: "40+", l: "Projects"    },
              { v: "12+", l: "Open Source" },
              ...
              <div key={s.l}>
                ...
                <p className="text-zinc-500 text-xs font-mono tracking-widest uppercase mt-1">{s.l}</p>
              </div>
            ))}
          </div>

          {/* TECH TAGS */}
          <div className={`flex flex-wrap gap-2 transition-all duration-700 delay-300 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
            {["React", "Node.js", "TypeScript", "MongoDB", "Tailwind"].map(tag => (
              <span key={tag} className="border border-zinc-800 text-zinc-500 text-xs font-mono px-3 py-1 rounded-full tracking-wider uppercase hover:border-red-800 hover:text-red-400 transition-colors duration-200">
                {tag}
              </span>
            ))}
          </div>
        ...
        {/* ══ COL 2: VINYL RECORD ══ */}
        <div className={`flex flex-col items-center gap-6 transition-all duration-700 delay-150 ${mounted ? "opacity-100 scale-100" : "opacity-0 scale-90"}`}>
          ...
          <div className="relative">
            ...
            {/* SPINNING DISC */}
            <div
              className="relative rounded-full bg-zinc-900 shadow-2xl"
              style={{
            ...
            >
              ...
              {Array.from({ length: 20 }).map((_, i) => (
                <div key={i} className="absolute rounded-full"
                  style={{
                    inset: `${8 + i * 7}px`,
                    border: `1px solid ${i % 3 === 0 ? "rgba(255,255,255,0.06)" : "rgba(255,255,255,0.025)"}`,
                  }}
                />
              ))}

              {/* Iridescent sheen */}
              <div className="absolute inset-0 rounded-full opacity-[0.07]"
                style={{ background: "conic-gradient(from 0deg, #ff0000, #ff8800, #ffff00, #00ff00, #0000ff, #8800ff, #ff0000)" }}
              />

              {/* CENTER LABEL */}
              <div className="absolute inset-[36%] rounded-full flex flex-col items-center justify-center"
                style={{ background: "linear-gradient(145deg, #7f1d1d, #991b1b)", boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.08)" }}>
                {/* inner rings on label */}
            ...
            <div className="absolute top-0 right-0 pointer-events-none"
              ...
              <div className="w-1 h-8 bg-zinc-600 rounded-full ml-2 mt-0.5"
        ...
        <div className={`flex flex-col gap-6 transition-all duration-700 delay-200 ${mounted ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}>

          {/* TRACKLIST */}
          <div className="rounded-2xl bg-zinc-900/60 border border-zinc-800 p-5">
            <div className="flex items-center gap-2 mb-5">
              <Disc3 size={13} className="text-amber-600" />
              <p className="text-zinc-500 text-xs font-mono tracking-widest uppercase">Featured Projects</p>
            </div>
            <div className="flex flex-col gap-2">
              {TRACKS.map((tr, i) => (
                ...
                >
                  ...
                  <span className="text-zinc-600 text-xs font-mono flex-shrink-0">{tr.duration}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-zinc-900/60 border border-zinc-800 p-5 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <p className="text-zinc-500 text-xs font-mono tracking-widest uppercase">Now Playing</p>
              </div>
              <span className="text-zinc-600 text-xs font-mono">{TRACKS[activeTrack].duration}</span>
            </div>
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={item.label} className="rounded-xl bg-zinc-900/40 border border-zinc-800 px-4 py-3">
                <p className="text-zinc-600 text-xs font-mono tracking-widest uppercase mb-1">{item.label}</p>
