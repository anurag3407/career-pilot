...
const OnboardingChecklist = ({ user, onComplete, onDismiss }) => {
  ...
  useEffect(() => {
    ...
    if (allCompleted && !showCelebration && !loading) {
      setShowCelebration(true);
      if (onComplete) onComplete();
      const timer = setTimeout(() => setShowCelebration(false), 5000);
      return () => clearTimeout(timer);
    }
  }, [items, showCelebration, loading, onComplete]);
              ...
              <motion.div
                ...
                className={`
                  ...
                  ${item.completed
                    ...
                    : 'bg-muted/50 hover:bg-muted border-border hover:border-primary/30 hover:shadow-md'
                  }
                `}
              >
                {/* Icon / Checkmark */}
