...
const generateBurnoutReport = (applicationData) => {
  ...
  if (!analysisResult.success) {
    return analysisResult; // Propagate validation error
  }
...
const validateInput = (input) => {
  ...
  if (input.rejections > input.applicationsSent) {
    return {
      valid: false,
      error: 'Rejections cannot exceed applications sent',
    };
  }

  if (input.interviews > input.applicationsSent) {
    return {
      valid: false,
      error: 'Interviews cannot exceed applications sent',
    };
  }

  if (input.responses > input.applicationsSent) {
    return {
      valid: false,
      error: 'Responses cannot exceed applications sent',
    };
  };
...
export { detectApplicationBurnout, generateBurnoutReport };
